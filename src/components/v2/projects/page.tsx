...
export function V2ProjectsPage() {
  ...
  const adminProjects = data?.adminProjects || []
  const githubProjects = data?.githubOnlyProjects || []
  const featuredProjects = adminProjects.filter(p => p.featured)
  const showcaseProjects = adminProjects.filter(p => !p.featured)
          ...
          <div className="px-6 sm:px-10 lg:px-16 xl:px-24 mb-16">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 sm:gap-10">
              {featuredProjects.map((project, i) => (
                <ProjectCard key={project.id} project={project} index={i} onClick={() => setSelectedProject(project)} isDark={isDark} accentColor={accentColor} textDesc={textDesc} bgCard={bgCard} borderLight={borderLight} hoverBorder={hoverBorder} />
              ))}
            </div>
          </div>
        )}
          ...
          <div className="px-6 sm:px-10 lg:px-16 xl:px-24">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 sm:gap-6">
              {githubProjects.map((project, i) => (
                <GithubOnlyCard key={project.src} project={project} index={i} isDark={isDark} textMuted={textMuted} />
              ))}
            </div>
          </div>
        )}
        ...
        {selectedProject && (
          ...
          >
            <motion.div
              ...
              className={`relative w-full max-w-3xl max-h-[90vh] overflow-y-auto ${modalBg} border rounded-3xl`}
            ...
            >
              ...
              <div className="p-6 sm:p-8">
                {selectedProject.hasImages && selectedProject.imageUrls.length > 0 && (
                  <div className="relative aspect-video rounded-2xl overflow-hidden mb-6">
                    <Image
                      src={selectedProject.imageUrls[0]}
                      alt={selectedProject.alt || selectedProject.name}
                      fill
                ...
                {selectedProject.subtitle && (
                  <p className={`${textDesc} leading-relaxed text-base mb-4`}>
                    {selectedProject.subtitle}
                  </p>
                )}

                {selectedProject.abt && (
                  <p className={`${textMuted} leading-relaxed text-sm mb-6`}>
                    {selectedProject.abt}
                  </p>
                )}
                ...
                {selectedProject.githubLanguages && Object.keys(selectedProject.githubLanguages).length > 0 && (
                  <div className="mb-6">
                    <div className="flex h-3 rounded-full overflow-hidden mb-3">
                      {Object.entries(selectedProject.githubLanguages)
                        .sort(([, a], [, b]) => b - a)
                        .map(([lang, pct]) => (
                          <div
                            key={lang}
...
function ProjectCard({ project, index, onClick, isDark, accentColor, textDesc, bgCard, borderLight, hoverBorder }: {
  project: ProjectData;
  index: number;
  onClick: () => void;
  isDark: boolean;
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -8 }}
    >
      {project.hasImages && project.imageUrls.length > 0 ? (
        ...
        <div className={`relative aspect-[16/10] overflow-hidden rounded-2xl ${bgCard} border ${borderLight} ${hoverBorder} transition-colors`}>
          <div className={`absolute inset-0 flex items-center justify-center ${isDark ? "bg-gradient-to-br from-neutral-900 to-black" : "bg-gradient-to-br from-neutral-100 to-white"}`}>
            <div className="text-center px-6">
              ...
              <p className={`text-lg font-bold ${isDark ? "text-white/60" : "text-black/60"}`}>{project.name}</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="mt-4 px-1">
        ...
        {project.subtitle && (
          <p className={`text-sm mt-1 line-clamp-1 ${textDesc}`}>{project.subtitle}</p>
        )}
...
function GithubOnlyCard({ project, index, isDark, textMuted }: {
  project: ProjectData;
  index: number;
  isDark: boolean;
  textMuted: string;
  ...
  return (
    <motion.a
      ...
      className={`block p-3 rounded-xl ${isDark ? "bg-white/5 hover:bg-white/10" : "bg-black/5 hover:bg-black/10"} border ${isDark ? "border-white/10" : "border-black/10"} transition-colors group`}
    >
      <div className="flex items-start justify-between gap-2 mb-2">
        <svg className={`w-5 h-5 flex-shrink-0 mt-0.5 ${isDark ? "text-white/40" : "text-black/40"}`} fill="currentColor" viewBox="0 0 24 24">
          <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/>
        </svg>
        {project.stars > 0 && (
          <span className={`text-xs ${textMuted}`}>★ {project.stars}</span>
        )}
      </div>
      <p className={`text-sm font-medium truncate ${isDark ? "text-white/80" : "text-black/80"} group-hover:${isDark ? "text-white" : "text-black"} transition-colors`}>
        {project.name}
      </p>
      {project.subtitle && (
        <p className={`text-xs mt-1 line-clamp-2 ${textMuted}`}>{project.subtitle}</p>
      )}
    </motion.a>
  )
}